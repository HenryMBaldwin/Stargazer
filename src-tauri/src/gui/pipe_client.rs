...
#[tauri::command]
pub async fn switch_database(id: String) -> Vec<(String, String, bool)> {
    let request = serde_json::to_string(&RequestType::SwitchDatabase(SwitchDatabaseRequest{id})).expect("Error: error serializing json.");
    let response = serde_json::from_str::<ResponseType>(&send_wait(&request).await.expect("Error connecting to pipe")).expect("Error deserializing json.");
    match response {
        ResponseType::SwitchDatabase(switch_database) => switch_database.databases,
        _ => { vec![(String::from("0"), String::from("Error: failed to switch databases."), true)] }
    }
}
